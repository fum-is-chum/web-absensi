...
class AttendanceControllerAPI extends Controller
{
    ...
    {
        $this->domain = 'http://192.168.8.118:8090/upload'; // Adjust accordingly
    }
    ...
    public function attendanceCheckIn(Request $request)
    {
        ...
        $response = Http::attach('photo', file_get_contents($image), $image->getClientOriginalName())->post($this->domain);
        $apiResponse = $response->body();
        $jsonApiResponse = json_decode($apiResponse);

        if(!$jsonApiResponse->success){
            return Helpers::ReturnResponseAPI(400, "No matching face", null);
        }

        $predictUsername = strtolower($jsonApiResponse->username);
        $username = Auth::user()->username;

        if($predictUsername != $username){
            return Helpers::ReturnResponseAPI(400, "Wajah tidak dikenal", null);
        }

        // Deteksi liveness
        if($jsonApiResponse->liveness == 'spoof'){
            return Helpers::ReturnResponseAPI(400, "Wajah Tidak Valid", null);
        }
        ...
        if($personalCalender->update()){
            return Helpers::ReturnResponseAPI(200,"Berhasil Check-in sebagai $username",null);
        }else{
    ...
    public function attendanceCheckOut(Request $request)
    {
        ...
        if(!$jsonApiResponse->success){
            return Helpers::ReturnResponseAPI(400, $jsonApiResponse->username, null);
        }
        ...
        if($personalCalender->update()){
            return Helpers::ReturnResponseAPI(200,"Berhasil Check-out sebagai $username",null);
        }else{
